...
export const syncService = {
  ...
  _lastLocalUpdate: 0,
  _lastRestoreTime: 0, 
  _lastBackupTime: Number(localStorage.getItem('last_backup_time') || '0'), 
  ...
  async getAllLocalData(incremental = false) {
    const userId = getCurrentUserId();
    const lastSync = incremental ? this._lastBackupTime : 0;
    ...
    const fullSubjects = await db.subjects.where('userId').equals(userId).toArray();
    const fullTopics = await db.topics.where('userId').equals(userId).toArray();
    
    const subjects = await ensureSyncId(db.subjects, fullSubjects, 'subject');
    const topics = await ensureSyncId(db.topics, fullTopics, 'topic');
    ...
    const filterIncremental = (items: any[]) => incremental ? items.filter(i => (i.updatedAt || 0) > lastSync) : items;
    ...
    const getStoreData = (store: any) => {
        if (!incremental) return store;
        return (store.updatedAt || 0) > lastSync ? store : undefined;
    };
    ...
    return {
      subjects: filterIncremental(subjects),
      topics: filterIncremental(topics),
      logs: filterIncremental(logs),
      timetable: filterIncremental(timetable),
      settings: filterIncremental(settings),
      resources: filterIncremental(resources),
      quiz: incremental ? {} : quizData, // Quiz data is small, but for now we skip incremental for localStorage items
      gamification: getStoreData({
        xp: gamification.xp,
        level: gamification.level,
        unlockedBadges: gamification.unlockedBadges,
        updatedAt: gamification.updatedAt || 0,
      }),
      achievements: getStoreData({
      ...
      timetableStore: getStoreData({
          completedSlots: useTimetableStore.getState().completedSlots,
          updatedAt: (useTimetableStore.getState() as any).updatedAt || Date.now()
      }),
      lastResetAt: incremental ? undefined : parseInt(localStorage.getItem('last_reset_at') || '0'),
      syllabus_version: incremental ? undefined : localStorage.getItem('syllabus_version')
    };
  },

  async backup(forceFull = false) {
    try {
      this._isSyncing = true;
      useSyncStore.getState().setSyncing(true);
      
      const incremental = !forceFull && this._lastBackupTime > 0;
      const data = await this.getAllLocalData(incremental);
      
      // Check if there's actually anything to sync in incremental mode
      if (incremental) {
          const hasChanges = Object.values(data).some(val => 
              Array.isArray(val) ? val.length > 0 : val !== undefined
          );
          if (!hasChanges) {
              console.log('Sync service: No changes to backup (Incremental)');
              return;
          }
      }

      // SAFETY GUARD: Prevent backing up an empty state if we haven't loaded yet.
      if (!this._isInitialized && (!data.subjects || data.subjects.length === 0) && !incremental) {
          console.warn('Sync service: Skipping backup - App not fully initialized and local data is empty.');
          return;
      }

      await api.post('/sync/backup', { isIncremental: incremental, data });
      ...
      console.log(`✅ Sync service: ${incremental ? 'INCREMENTAL' : 'FULL'} BACKUP SUCCESS`);
    } catch (err) {
