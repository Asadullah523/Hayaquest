    ...
    const quizContent = showResult ? (
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center sm:p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div className="bg-white dark:bg-slate-800 w-full max-w-4xl h-[100dvh] sm:h-[90vh] rounded-none sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden">
                {/* Header */}
                <div className="p-3 lg:p-3 2xl:p-6 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between shrink-0">
                    <div className="min-w-0 flex-1">
                        <h2 className="text-sm lg:text-lg 2xl:text-xl font-bold text-gray-900 dark:text-white truncate">{paper.title}</h2>
                        <p className="text-[10px] lg:text-[11px] 2xl:text-sm text-gray-500 dark:text-gray-400">
                ...
                >
                    <div className="max-w-3xl mx-auto">
                        <h3 className="text-base lg:text-lg 2xl:text-2xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-8 leading-relaxed">
                            ...
                            {currentQuestion.options.map((option, index) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`
                                            w-full p-3 lg:p-3 2xl:p-6 text-left rounded-xl sm:rounded-2xl border-2 transition-all flex items-center gap-3 sm:gap-4 group
                                            ${isSelected 
                                    ...
                                    >
                                        <div className={`
                                            w-6 h-6 sm:w-8 sm:h-8 lg:w-7 lg:h-7 2xl:w-8 2xl:h-8 rounded-full border-2 flex items-center justify-center font-bold text-[10px] lg:text-xs 2xl:text-sm shrink-0 transition-colors
                                            ${isSelected 
                                        ...
                                        <span className={`text-[13px] lg:text-base 2xl:text-lg ${isSelected ? 'text-primary font-medium' : 'text-gray-700 dark:text-gray-200'}`}>
                                            {option}
                                        </span>
                                    </button>
                                );
                ...
                {/* Footer Navigation */}
                <div className="p-3 lg:p-3 2xl:p-6 border-t border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50 flex justify-between items-center shrink-0 mb-[env(safe-area-inset-bottom)]">
                    <button
                        onClick={handlePrevious}
                        disabled={currentQuestionIndex === 0}
                        className="px-3.5 sm:px-6 lg:px-4 py-2 sm:py-3 lg:py-2 rounded-xl font-bold text-xs lg:text-[11px] 2xl:text-sm text-gray-600 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors"
                    >
                        Previous
                    </button>
                    ...
                    <div className="flex gap-2">
                        {currentQuestionIndex === questions.length - 1 ? (
                            <button
                                onClick={handleSubmit}
                                className="px-5 sm:px-8 lg:px-5 py-2 sm:py-3 lg:py-2 bg-primary text-white rounded-xl font-bold hover:bg-primary/90 shadow-lg shadow-primary/30 transition-all flex items-center gap-2 text-xs lg:text-[11px] 2xl:text-sm"
                            >
                            ...
                            <button
                                onClick={handleNext}
                                className="px-5 sm:px-8 lg:px-5 py-2 sm:py-3 lg:py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl font-bold hover:opacity-90 transition-all flex items-center gap-2 text-xs lg:text-[11px] 2xl:text-sm"
                            >
