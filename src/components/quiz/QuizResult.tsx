    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center sm:p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div className="bg-white dark:bg-slate-800 w-full max-w-4xl h-full sm:h-[90vh] rounded-none sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4 lg:p-6 2xl:p-8">
                    {/* Score Summary */}
                    <div className="flex flex-col items-center mb-8 sm:mb-12">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 w-full max-w-3xl lg:max-w-4xl">
                            <div className="p-3 lg:p-3 2xl:p-4 rounded-2xl bg-green-50 dark:bg-green-900/20 text-center text-green-700 dark:text-green-300">
                                <CheckCircle className="w-5 h-5 lg:w-6 lg:h-6 2xl:w-8 2xl:h-8 mx-auto mb-1 sm:mb-2 opacity-80" />
                                <div className="text-xl lg:text-lg 2xl:text-2xl font-bold">{correctCount}</div>
                                <div className="text-xs lg:text-[10px] 2xl:text-sm opacity-80 font-bold uppercase tracking-tight">Correct</div>
                            </div>
                            <div className="p-3 lg:p-3 2xl:p-4 rounded-2xl bg-red-50 dark:bg-red-900/20 text-center text-red-700 dark:text-red-300">
                                <XCircle className="w-5 h-5 lg:w-6 lg:h-6 2xl:w-8 2xl:h-8 mx-auto mb-1 sm:mb-2 opacity-80" />
                                <div className="text-xl lg:text-lg 2xl:text-2xl font-bold">{Object.keys(answers).length - correctCount}</div>
                                <div className="text-xs lg:text-[10px] 2xl:text-sm opacity-80 font-bold uppercase tracking-tight">Incorrect</div>
                            </div>
                            <div className="p-3 lg:p-3 2xl:p-4 rounded-2xl bg-blue-50 dark:bg-blue-900/20 text-center text-blue-700 dark:text-blue-300">
                                <Target className="w-5 h-5 lg:w-6 lg:h-6 2xl:w-8 2xl:h-8 mx-auto mb-1 sm:mb-2 opacity-80" />
                                <div className="text-xl lg:text-lg 2xl:text-2xl font-bold">{totalQuestions}</div>
                                <div className="text-xs lg:text-[10px] 2xl:text-sm opacity-80 font-bold uppercase tracking-tight">Total Qs</div>
                            </div>
                            <div className="p-3 lg:p-3 2xl:p-4 rounded-2xl bg-purple-50 dark:bg-purple-900/20 text-center text-purple-700 dark:text-purple-300">
                                <Target className="w-5 h-5 lg:w-6 lg:h-6 2xl:w-8 2xl:h-8 mx-auto mb-1 sm:mb-2 opacity-80" />
                                <div className="text-xl lg:text-lg 2xl:text-2xl font-bold">
                                    {Object.keys(answers).length > 0 
                                        ? Math.round((correctCount / Object.keys(answers).length) * 100) 
                                        : 0}%
                                </div>
                                <div className="text-xs lg:text-[10px] 2xl:text-sm opacity-80 font-bold uppercase tracking-tight">Accuracy</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 max-w-3xl mx-auto">
                        ...
                        {paper.questions
                            ...
                            return (
                                <div key={q.id} className={`p-4 lg:p-5 2xl:p-6 rounded-2xl border ${isCorrect ? 'border-green-200 bg-green-50/30' : 'border-red-200 bg-red-50/30'} `}>
