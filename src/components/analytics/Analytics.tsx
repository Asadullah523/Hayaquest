...
export const Analytics: React.FC = () => {
  ...
  const subjectData = useMemo(() => {
     ...
     return Array.from(map.entries()).map(([id, seconds]) => {
         ...
         return {
             name: id === 0 ? 'General' : (subject?.name || 'Unknown'),
             color: id === 0 ? '#94a3b8' : (subject?.color || '#ccc'),
             seconds,
  ...
  const DonutChart = () => {
      ...
      return (
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-8 lg:gap-6 2xl:gap-8 justify-center">
              <div 
                className="w-40 h-40 lg:w-32 lg:h-32 2xl:w-40 2xl:h-40 aspect-square flex-shrink-0 rounded-full relative"
                style={{ background: conicStyle }}
              >
                  <div className="absolute inset-4 lg:inset-3 2xl:inset-4 bg-white/90 dark:bg-slate-800/90 rounded-full flex items-center justify-center backdrop-blur-sm">
                      <div className="text-center">
                          <p className="text-2xl lg:text-xl 2xl:text-2xl font-bold">{(totalStudySeconds / 3600).toFixed(1)}</p>
  ...
  return (
    <div className="page-transition space-y-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <h1 className="text-3xl lg:text-2xl 2xl:text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    Insights <TrendingUp className="text-primary" />
                </h1>
                <p className="text-gray-500 dark:text-gray-400 mt-1 lg:text-sm 2xl:text-base">
        ...
        {/* Key Stats Cards */}
        <div className="grid grid-cols-4 md:grid-cols-2 lg:grid-cols-4 gap-1.5 sm:gap-6 lg:gap-4 2xl:gap-6">
            <div className="glass-card p-1.5 sm:p-6 lg:p-4 2xl:p-6 rounded-xl sm:rounded-3xl flex flex-col sm:flex-row items-center sm:items-center gap-1 sm:gap-4 text-center sm:text-left">
                <div className="p-1.5 sm:p-4 lg:p-3 2xl:p-4 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-lg sm:rounded-2xl">
                    <Clock size={12} className="sm:w-7 sm:h-7 lg:w-5 lg:h-5 2xl:w-7 2xl:h-7" />
                </div>
                <div className="min-w-0">
                    ...
                    <p className="text-[9px] sm:text-2xl lg:text-xl 2xl:text-2xl font-black text-gray-900 dark:text-white dark:text-neon-indigo truncate">{formatTimeDetailed(totalStudySeconds)}</p>
                </div>
            </div>
            
            <div className="glass-card p-1.5 sm:p-6 lg:p-4 2xl:p-6 rounded-xl sm:rounded-2xl flex flex-col sm:flex-row items-center sm:items-center gap-1 sm:gap-4 text-center sm:text-left">
                <div className="p-1.5 sm:p-4 lg:p-3 2xl:p-4 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-lg sm:rounded-2xl">
                    <CheckCircle2 size={12} className="sm:w-7 sm:h-7 lg:w-5 lg:h-5 2xl:w-7 2xl:h-7" />
                </div>
                <div className="min-w-0">
                    ...
                    <p className="text-[9px] sm:text-2xl lg:text-xl 2xl:text-2xl font-black text-gray-900 dark:text-white dark:text-neon-emerald">{topicStats.completed}</p>
                </div>
            </div>
            
            <div className="glass-card p-1.5 sm:p-6 lg:p-4 2xl:p-6 rounded-xl sm:rounded-2xl flex flex-col sm:flex-row items-center sm:items-center gap-1 sm:gap-4 text-center sm:text-left">
                <div className="p-1.5 sm:p-4 lg:p-3 2xl:p-4 bg-amber-100 dark:bg-amber-900/30 text-amber-600 rounded-lg sm:rounded-2xl">
                    <Trophy size={12} className="sm:w-7 sm:h-7 lg:w-5 lg:h-5 2xl:w-7 2xl:h-7" />
                </div>
                <div className="min-w-0 w-full">
                     ...
                     <p className="text-[9px] sm:text-xl lg:text-lg 2xl:text-xl font-black text-gray-900 dark:text-white dark:text-neon-orange truncate">
             ...
             <div className="glass-card p-1.5 sm:p-6 lg:p-4 2xl:p-6 rounded-xl sm:rounded-2xl flex flex-col sm:flex-row items-center sm:items-center gap-1 sm:gap-4 text-center sm:text-left">
                <div className="p-1.5 sm:p-4 lg:p-3 2xl:p-4 bg-purple-100 dark:bg-purple-900/30 text-purple-600 rounded-lg sm:rounded-2xl">
                    <BarChart2 size={12} className="sm:w-7 sm:h-7 lg:w-5 lg:h-5 2xl:w-7 2xl:h-7" />
                </div>
                <div className="min-w-0">
                     ...
                     <p className="text-[9px] sm:text-2xl lg:text-xl 2xl:text-2xl font-black text-gray-900 dark:text-white dark:text-neon-purple">{(currentWeekSeconds / 3600).toFixed(1)}h</p>
                </div>
            </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-4 2xl:gap-6">
            {/* Weekly Activity */}
            <div className="glass-card p-6 lg:p-4 2xl:p-6 rounded-3xl">
