...
export const MyCollection: React.FC = () => {
  ...
  return (
    <div className="space-y-4 md:space-y-6 max-w-6xl mx-auto pb-20 md:pb-0">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 mb-4 md:mb-8 lg:mb-4 2xl:mb-8">
      ...
      <div className="mt-4 md:mt-8 px-1">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-4 2xl:gap-6"
          >
...
const WordCard = ({ word, onRemove, type }: { word: any, onRemove: () => void, type: 'favorite' | 'learned' }) => (
  <div className="group bg-white dark:bg-slate-800 p-6 lg:p-4 2xl:p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-xl shadow-slate-200/50 dark:shadow-none hover:border-indigo-500/50 transition-all flex flex-col justify-between h-full relative overflow-hidden">
...
const StoryCard = ({ story, onRemove }: { story: any, onRemove: () => void }) => (
  <div className="group bg-white dark:bg-slate-800 p-6 lg:p-4 2xl:p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-xl shadow-slate-200/50 dark:shadow-none hover:border-indigo-500/50 transition-all flex flex-col h-full relative overflow-hidden">
    <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
      <button 
        onClick={onRemove}
        className="p-2 bg-red-50 dark:bg-red-900/20 text-red-500 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors"
