...
export const Achievements: React.FC = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-8 animate-fade-in max-w-7xl mx-auto px-4">
      ...
      <div className="relative overflow-hidden rounded-2xl sm:rounded-[2rem] bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-3 lg:p-4 2xl:p-10 text-center shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <h1 className="text-xl sm:text-3xl lg:text-2xl 2xl:text-5xl font-black text-white mb-1 sm:mb-2 tracking-tight drop-shadow-lg uppercase">
          ...
          <div className="h-8 lg:h-10 2xl:h-16 flex items-center justify-center max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-4 md:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-6 lg:gap-3 2xl:gap-6 px-1 sm:px-2">
        {[
          { 
            icon: <Trophy />, 
          ...
          { icon: <Target />, label: 'Completed Topics', value: topics.filter(t => t.isCompleted).length, color: 'text-purple-500', bg: 'bg-purple-50 dark:bg-purple-900/10' },
            ...
            <div 
              ...
              className={`glass-card hover:scale-105 transition-all duration-300 rounded-xl sm:rounded-[2rem] p-1.5 lg:p-3 2xl:p-6 border-2 flex flex-col items-center text-center group
                ${stat.onClick ? 'cursor-pointer' : ''}
                ${stat.active ? 'border-yellow-400 bg-yellow-400/5 shadow-[0_0_20px_rgba(250,204,21,0.2)]' : 'border-transparent shadow-lg'}`}
            >
              <div className={`w-8 h-8 sm:w-16 lg:w-10 lg:h-10 2xl:w-16 2xl:h-16 rounded-lg sm:rounded-2xl flex items-center justify-center mb-1 lg:mb-2 2xl:mb-4 transition-all duration-300 shadow-inner
                ${stat.active ? 'bg-yellow-400 text-yellow-900 scale-110 rotate-3' : `${stat.bg} ${stat.color} group-hover:scale-110`}`}>
                {React.cloneElement(stat.icon as React.ReactElement<any>, { size: 14, className: "sm:w-7 sm:h-7 lg:w-5 lg:h-5 2xl:w-7 2xl:h-7" })}
              </div>
              <div className={`text-xs sm:text-3xl lg:text-lg 2xl:text-3xl font-black mb-0 sm:mb-1 ${stat.active ? 'text-yellow-600 dark:text-yellow-400' : 'text-slate-900 dark:text-white'}`}>
                {stat.value}
              </div>
      ...
      <div className="space-y-12 lg:space-y-4 2xl:space-y-12 pb-20 lg:pb-32 2xl:pb-20">
        {(Object.entries(categorizedAchievements) as [AchievementCategory, any[]][]).map(([category, list]) => {
          ...
          return (
            <div key={category} className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-6 lg:gap-3 2xl:gap-6">
