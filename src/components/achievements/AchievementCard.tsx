...
export const AchievementCard: React.FC<AchievementCardProps> = ({ achievement, unlocked, unlockedAt }) => {
  ...
  if (unlocked) {
    return (
      <div
        className={`glass-card rounded-xl sm:rounded-[2.5rem] p-1.5 lg:p-3 2xl:p-8 transition-all duration-700 relative overflow-hidden group border-2
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className={`mb-0.5 sm:mb-6 lg:mb-2 2xl:mb-6 bg-gradient-to-r ${styles.badge} text-white px-1 sm:px-4 py-0 sm:py-1.5 rounded-full text-[4px] sm:text-[10px] font-black uppercase tracking-tight sm:tracking-[0.2em] flex items-center gap-0.5 sm:gap-2 shadow-lg scale-100 sm:scale-110 lg:scale-90 2xl:scale-110`}>
            <Sparkles size={4} className="animate-pulse sm:w-3 sm:h-3" />
            {achievement.tier} Reward
          </div>

          {/* Large Trophy Icon */}
          <div className={`relative mb-1 sm:mb-6 lg:mb-3 2xl:mb-6 group-hover:scale-110 transition-transform duration-500`}>
            <div className={`absolute inset-0 bg-gradient-to-br ${styles.badge} opacity-20 blur-2xl rounded-full animate-pulse-subtle`} />
            <div className={`w-8 h-8 sm:w-28 sm:h-28 lg:w-14 lg:h-14 2xl:w-28 2xl:h-28 rounded-md sm:rounded-3xl flex items-center justify-center text-lg sm:text-6xl lg:text-2xl 2xl:text-6xl shadow-2xl bg-white/10 backdrop-blur-sm border border-white/20 relative z-10 overflow-hidden`}>
              {achievement.icon.startsWith('/') ? (
                <img src={achievement.icon} alt={achievement.name} className="w-5 h-5 sm:w-20 sm:h-20 lg:w-10 lg:h-10 2xl:w-20 2xl:h-20 object-contain drop-shadow-2xl" />
              ) : (
                <span className="drop-shadow-lg text-sm sm:text-6xl lg:text-2xl 2xl:text-6xl">{achievement.icon}</span>
              )}
            </div>
          ...
          {/* Achievement Name */}
          <h3 className="font-black text-[7px] sm:text-2xl lg:text-base 2xl:text-2xl mb-0 sm:mb-2 text-slate-900 dark:text-white tracking-tight drop-shadow-sm uppercase line-clamp-1">
            {achievement.name}
          </h3>
          
          <p className="hidden sm:block text-[11px] lg:text-xs font-bold text-slate-600 dark:text-slate-400 leading-relaxed mb-6 max-w-[200px]">
  ...
  return (
    <div
      className="glass-card rounded-2xl sm:rounded-[2rem] p-2 lg:p-3 2xl:p-6 transition-all duration-500 relative overflow-hidden group border-2 border-transparent bg-slate-50/50 dark:bg-slate-900/20 opacity-80 grayscale-[0.3] hover:opacity-100 hover:grayscale-0"
    >
      <div className="relative z-10">
        ...
        <div className="mb-0.5 sm:mb-6 lg:mb-3 2xl:mb-6">
          <h3 className="font-black text-[8px] sm:text-lg lg:text-sm 2xl:text-lg mb-0 sm:mb-1 text-slate-500 dark:text-slate-400 line-clamp-1">
            {achievement.name}
          </h3>
        ...
        <div className="space-y-1 sm:space-y-3">
          ...
          <div className="mt-0.5 sm:mt-4 bg-slate-900/5 dark:bg-slate-100/5 p-0.5 lg:p-2 2xl:p-3 rounded-md sm:rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
            <p className="text-[5px] sm:text-[10px] lg:text-[11px] text-slate-500 font-bold leading-tight sm:leading-relaxed line-clamp-2">
