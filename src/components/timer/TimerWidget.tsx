...
export const TimerWidget: React.FC = () => {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center bg-white dark:bg-slate-900 rounded-3xl 2xl:rounded-[3rem] p-3 sm:p-4 2xl:p-8 shadow-2xl border border-slate-100 dark:border-slate-800 transition-all duration-500 hover:shadow-indigo-500/10 h-full min-h-[160px] sm:min-h-[220px] lg:min-h-[200px] 2xl:min-h-[320px]">
      
      {/* Settings Toggle */}
      <button 
        onClick={() => setShowSettings(!showSettings)}
        className="absolute top-2 right-2 sm:top-2.5 sm:right-2.5 lg:top-3 lg:right-3 2xl:top-6 2xl:right-6 p-1.5 sm:p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 transition-all z-20"
      >
        {showSettings ? <X size={18} /> : <Settings size={18} />}
      </button>

      {/* Mode Indicator */}
      <div className="absolute top-6 sm:top-6 lg:top-7 2xl:top-8 left-1/2 -translate-x-1/2 z-10 flex gap-1 sm:gap-1.5 p-0.5 sm:p-1 bg-slate-50 dark:bg-slate-800 rounded-full">
        {(['focus', 'shortBreak', 'longBreak'] as const).map((m) => (
          <button
            key={m}
            onClick={() => setMode(m)}
            className={`px-1.5 py-1 sm:px-2.5 lg:px-2 2xl:px-4 rounded-full text-[8px] sm:text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${
              mode === m 
      ...
      <div className="mt-14 sm:mt-12 lg:mt-16 w-full max-w-[140px] sm:max-w-[200px] z-20">
        {mode === 'focus' && (
      ...
      {/* Circular Timer */}
      <div className="relative w-[90px] h-[90px] sm:w-[110px] sm:h-[110px] md:w-36 md:h-36 lg:w-40 lg:h-40 2xl:w-52 2xl:h-52 flex items-center justify-center mt-3 sm:mt-4 2xl:mt-6">
        {/* Background Circle */}
        ...
        {/* Inner Content */}
        <div className="relative text-center space-y-0.5 sm:space-y-1 md:space-y-1 2xl:space-y-2 z-10">
          <div className="flex justify-center mb-0 sm:mb-1 md:mb-1 2xl:mb-2">
            {mode === 'focus' ? (
          ...
          <div className="text-xl sm:text-3xl md:text-3xl 2xl:text-5xl font-black text-slate-800 dark:text-white tabular-nums tracking-tight">
            {formatTime(timeLeft)}
          </div>
          {isActive && (
      ...
      {/* Controls */}
      <div className="flex items-center gap-2 sm:gap-4 md:gap-4 2xl:gap-6 mt-3 sm:mt-4 2xl:mt-10">
        <button
          onClick={reset}
        ...
        <button
          ...
          className={`w-14 h-14 sm:w-16 sm:h-16 lg:w-14 lg:h-14 2xl:w-24 2xl:h-24 rounded-full flex items-center justify-center shadow-xl hover:bg-indigo-700 transition-all active:scale-95 z-10 ${
            isActive 
