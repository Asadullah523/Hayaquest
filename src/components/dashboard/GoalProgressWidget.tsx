...
export const GoalProgressWidget: React.FC = () => {
  ...
  return (
    <div className={clsx(
      "bg-white/80 dark:bg-slate-950/40 backdrop-blur-md px-2.5 sm:px-5 md:px-6 py-1.5 sm:py-4 md:py-5 rounded-lg sm:rounded-2xl shadow-sm border border-white/20 dark:border-slate-700/50 flex-1 relative overflow-hidden group/goal transition-all duration-500",
      goalPercentage === 100 && "goal-met-glow goal-met-shine"
    )}>
      <div className="flex items-center justify-between mb-1 sm:mb-3">
        <p className="text-[6px] sm:text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-tight sm:tracking-widest leading-none">Goal</p>
        {goalPercentage === 100 && (
      ...
      <div className="flex items-end gap-1 sm:gap-2 mb-1 sm:mb-3">
        <p className={clsx(
          "text-xs sm:text-2xl md:text-3xl font-black whitespace-nowrap",
          goalPercentage === 100 && "text-premium-shine"
        )}>
          {formatStudyTime(todaysStudySeconds)}
        </p>
        <p className="text-[7px] sm:text-xs font-bold text-slate-400 mb-0.5 truncate">/ {dailyGoalMinutes}m</p>
      </div>
      <div className="w-full h-0.5 sm:h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
        <div 
          className={clsx(
