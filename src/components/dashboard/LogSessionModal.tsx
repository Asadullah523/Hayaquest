...
export const LogSessionModal: React.FC<LogSessionModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-white/20 overflow-hidden animate-premium-fade-in max-h-[95dvh] lg:max-h-[90vh] flex flex-col">
        
        {/* Header */}
        <div className="p-5 lg:p-4 2xl:p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h2 className="text-xl lg:text-lg 2xl:text-2xl font-black text-slate-800 dark:text-white">Log Study Session</h2>
            <p className="text-[10px] lg:text-[9px] 2xl:text-sm text-slate-400 font-bold uppercase tracking-widest">Step {step} of 3</p>
          </div>
        ...
        {/* Content */}
        <div className="p-5 lg:p-4 2xl:p-8 min-h-[300px] lg:min-h-[250px] 2xl:min-h-[400px]">
          {/* Error Message Alert */}
          {errorMessage && (
            ...
            <>
              {step === 1 && (
                <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 gap-3 max-h-[350px] lg:max-h-[220px] 2xl:max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                    {displaySubjects.map(subject => (
                      <button
                        ...
                        className={`flex items-center justify-between p-4 lg:p-3 2xl:p-4 rounded-2xl border transition-all hover:scale-[1.02] active:scale-95 ${
              ...
              {step === 2 && (
                <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="space-y-3 max-h-[350px] lg:max-h-[220px] 2xl:max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                    {hierarchy.length > 0 ? (
                       hierarchy.map(group => (
                                  ...
                                  <div className="grid grid-cols-1 gap-2 pl-2">
                                      {group.topics.map(topic => (
                                          <button
                                            ...
                                            className={`w-full flex items-center justify-between p-3 lg:p-2.5 2xl:p-3 rounded-xl border transition-all ${
              ...
              {step === 3 && (
                <div className="space-y-10 animate-fade-in py-4">
                  <div className="text-center space-y-2">
                    ...
                    <div className="text-6xl lg:text-4xl 2xl:text-6xl font-black text-indigo-600 tracking-tighter tabular-nums">
                        {duration}<span className="text-2xl lg:text-xl 2xl:text-2xl text-slate-300">m</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 px-2 md:px-4">
                    ...
                    />
                    <div className="flex justify-between text-[8px] lg:text-[7px] 2xl:text-[10px] font-black text-slate-400 uppercase tracking-widest">
                       <span>5m</span>
                       <span>60m</span>
                       <span>120m</span>
                    </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-6 lg:p-4 2xl:p-6 rounded-3xl border border-slate-100 dark:border-slate-800 flex items-center gap-4 lg:gap-3 2xl:gap-4">
                     <div className="w-12 h-12 lg:w-10 lg:h-10 2xl:w-12 2xl:h-12 bg-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 shadow-inner">
                        <Calendar size={24} className="lg:w-5 lg:h-5 2xl:w-6 2xl:h-6" />
                     </div>
                     <div>
                        <p className="text-[10px] lg:text-[9px] 2xl:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Logging for</p>
                        <p className="text-sm lg:text-xs 2xl:text-sm font-bold text-slate-800 dark:text-white mt-1">Today, {new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} • {new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</p>
                     </div>
                  </div>
                </div>
              )}
        ...
        {/* Footer */}
        {!isSuccess && (
          <div className="p-5 lg:p-4 2xl:p-8 pt-0 flex gap-3 md:gap-4">
            {step > 1 && (
              <button 
                onClick={handleBack}
                className="flex-1 py-3 lg:py-2.5 2xl:py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl md:rounded-2xl font-bold hover:bg-slate-200 transition-all active:scale-95 text-sm lg:text-xs 2xl:text-base"
              >
                Back
              </button>
            )}
            <button 
              ...
              className={`flex-[2] py-3 lg:py-2.5 2xl:py-4 rounded-xl md:rounded-2xl font-bold flex items-center justify-center gap-2 shadow-xl transition-all active:scale-95 text-sm lg:text-xs 2xl:text-base ${
