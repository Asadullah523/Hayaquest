...
export const QuestStats: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      {/* Level & Mastery Card */}
      <div className="relative group overflow-hidden rounded-3xl 2xl:rounded-[2.5rem] bg-slate-900 p-5 lg:p-6 2xl:p-8 text-white shadow-2xl transition-all duration-500 hover:-translate-y-1">
          ...
          <div className="flex justify-between items-start mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-xl lg:text-2xl font-black tracking-tight dark:text-neon-indigo">Level {level}</h3>
              </div>
            </div>
      ...
      <div className="relative group overflow-hidden rounded-3xl 2xl:rounded-[2.5rem] bg-white dark:bg-slate-900 p-5 lg:p-6 2xl:p-8 border border-slate-100 dark:border-slate-800 shadow-xl transition-all duration-500 hover:-translate-y-1">
         <div className="flex items-center gap-6">
            <div className="relative">
              <div className="absolute inset-0 bg-orange-500/20 blur-xl rounded-full animate-pulse" />
              <div className="relative w-14 h-14 lg:w-16 lg:h-16 2xl:w-20 2xl:h-20 bg-gradient-to-br from-orange-400 to-rose-600 rounded-2xl lg:rounded-3xl 2xl:rounded-[2rem] flex items-center justify-center shadow-2xl shadow-orange-500/20">
                <Flame size={32} className="text-white fill-current animate-bounce group-hover:scale-110 transition-transform" />
              </div>
            </div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-500 mb-1">Persistence</p>
              <h3 className="text-2xl lg:text-3xl 2xl:text-4xl font-black text-slate-800 dark:text-white dark:text-neon-orange tracking-tighter tabular-nums">{streak} Days</h3>
              <p className="text-[10px] lg:text-xs font-bold text-slate-400 mt-1">Study Momentum Active</p>
            </div>
         </div>
      </div>

      {/* Champion's Hall / Badges Card */}
      <div className="relative group overflow-hidden rounded-2xl sm:rounded-3xl 2xl:sm:rounded-[2.5rem] p-4 lg:p-6 2xl:p-8 border border-slate-100 dark:border-slate-800 shadow-xl transition-all duration-500 hover:-translate-y-1 sm:col-span-2 lg:col-span-1">
