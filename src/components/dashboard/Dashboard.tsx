...
export const Dashboard: React.FC = () => {
  ...
  const getGreeting = () => {
    ...
    return (
      <div className="page-transition space-y-2 sm:space-y-4 lg:space-y-2 2xl:space-y-8">
      ...
      <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 md:gap-6 2xl:gap-8 min-h-[140px] sm:min-h-[160px] 2xl:min-h-[220px] lg:mb-6 2xl:mb-0">
        <div className="flex-1 glass-card rounded-xl sm:rounded-2xl md:rounded-3xl 2xl:rounded-[2.5rem] px-4 py-3 sm:px-4 sm:py-3 lg:p-4 2xl:p-8 relative overflow-hidden group">
           ...
           <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-2 sm:gap-4 md:gap-8">
              <div className="space-y-1 sm:space-y-2 md:space-y-3 w-full">
                 <div className="flex items-center gap-1.5 sm:gap-2 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-[8px] sm:text-[9px] md:text-[10px] bg-white/50 dark:bg-slate-900/50 w-fit px-2 sm:px-3 py-0.5 sm:py-1 rounded-full border border-slate-200 dark:border-slate-700">
                   ...
                   <div className="flex flex-row items-center gap-2 sm:gap-2.5 md:gap-4 lg:gap-4 2xl:gap-6">
                     <div className="w-14 h-14 sm:w-16 sm:h-16 md:w-16 md:h-16 lg:w-20 lg:h-20 2xl:w-24 2xl:h-24 rounded-full border-2 sm:border-3 md:border-3 2xl:border-4 border-white dark:border-slate-700 shadow-2xl overflow-hidden shrink-0 animate-float">
                       <img 
                         key={avatar}
                      ...
                      <div className="flex flex-col flex-1 min-w-0">
                        <div className="flex flex-wrap items-baseline gap-1 sm:gap-1.5 md:gap-2 mb-0.5 sm:mb-1 md:mb-1 2xl:mb-2">
                          <h1 className="text-xl sm:text-2xl md:text-2xl lg:text-3xl 2xl:text-4xl font-bold text-black dark:text-white tracking-tight leading-none drop-shadow-sm" style={{ fontFamily: "'Amatic SC', cursive" }}>
                            {getGreeting()},
                          </h1>
                          <h1 className="text-xl sm:text-2xl md:text-2xl lg:text-3xl 2xl:text-4xl font-bold text-black dark:text-white tracking-tight leading-none drop-shadow-sm" style={{ fontFamily: "'Dancing Script', cursive" }}>
                            <span className="relative inline-block text-primary">
                              {displayName}!
                              <span className="absolute bottom-0 left-0 w-3/4 h-1.5 sm:h-2 md:h-1.5 2xl:h-2 bg-primary/40 -z-10 rounded-full"></span>
                            </span> 🚀
                          </h1>
                        </div>
                        
                        <div className="max-w-lg mt-0.5">
                          <p className="text-slate-800 dark:text-slate-200 font-medium xl:font-normal 2xl:font-medium italic leading-snug text-[11px] sm:text-xs md:text-sm xl:text-[11px] 2xl:text-base mb-0.5">" {quote.text}"</p>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 gap-2 sm:gap-2.5 md:gap-3 lg:gap-4 w-full">
                  <div className="bg-white/80 dark:bg-slate-950/40 backdrop-blur-md px-2 py-1.5 sm:px-2.5 sm:py-2 md:px-3 md:py-2.5 lg:px-4 lg:py-3 2xl:px-6 2xl:py-5 rounded-lg sm:rounded-xl md:rounded-2xl shadow-sm border border-white/20 dark:border-slate-700/50 flex-1">
                     <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 mb-1 sm:mb-1.5">
                        ...
                        <div className="flex-1 min-w-0">
                           <p className="text-[7px] sm:text-[8px] md:text-[8px] lg:text-[9px] 2xl:text-[10px] font-black text-slate-400 uppercase tracking-tight sm:tracking-widest leading-none mb-0.5">Streak</p>
                           <p className="text-[11px] sm:text-xs md:text-base lg:text-xl 2xl:text-2xl font-black text-slate-900 dark:text-white truncate">{streak} Days</p>
                        </div>
                     </div>
                     <DailyProgressLights />
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-4 2xl:gap-8 mt-4 sm:mt-6 lg:mt-8 2xl:mt-8 relative z-10">
        
        {/* Sidebar - Timer, SessionStats, RecentActivity on desktop */}
        <aside className="lg:col-span-4 lg:sticky lg:top-4 h-fit space-y-4 sm:space-y-6 md:space-y-6 2xl:space-y-8">
          <TimerWidget />
          ...
          <section className="lg:hidden space-y-4 sm:space-y-6">
        ...
        {/* Main Content Area */}
        <div className="lg:col-span-8 space-y-4 sm:space-y-6 md:space-y-6 2xl:space-y-8">
          ...
          <div className="hidden lg:grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 lg:gap-3 2xl:gap-6">
            <div className="col-span-full flex items-center justify-between px-1 mb-1 lg:mb-1 2xl:mb-2">
                <h2 className="text-lg lg:text-lg 2xl:text-xl font-black text-slate-800 dark:text-white flex items-center gap-2 lg:gap-3">
                ...
                <div className="h-1 flex-1 mx-4 xl:mx-6 bg-gradient-to-r from-indigo-200 via-purple-200 to-transparent dark:from-indigo-900 dark:via-purple-900 rounded-full max-w-[200px]" />
            </div>
            ...
            {[
              ...
              const props = stat.link ? { to: stat.link } : { onClick: stat.action };
              return (
                <Wrapper key={i} {...props as any} className={`glass-card rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-4 xl:p-4 2xl:p-6 text-left group hover:-translate-y-1 hover:shadow-xl ${(stat as any).shadow} transition-all block cursor-pointer relative overflow-hidden`}>
                  <div className="absolute inset-0 bg-gradient-to-br from-white/50 to-transparent dark:from-white/5 opacity-0 group-hover:opacity-100 transition-opacity" />
                  ...
                  <p className="text-lg sm:text-xl md:text-xl 2xl:text-2xl font-black mb-0.5 sm:mb-1 relative z-10">{stat.value}</p>
                  <p className="text-[8px] sm:text-[9px] md:text-[9px] 2xl:text-[10px] font-black text-slate-400 uppercase tracking-widest relative z-10">{stat.label}</p>
                </Wrapper>
              );
            })}
          </div>

          <section className="hidden lg:block space-y-4 sm:space-y-6 md:space-y-6 2xl:space-y-8">
            <div className="flex items-center justify-between px-1 sm:px-2">
              <h2 className="text-base sm:text-lg xl:text-xl font-black text-slate-800 dark:text-white flex items-center gap-2 sm:gap-3">
                <Calendar size={16} className="sm:w-[18px] sm:h-[18px]" /> Today's Focus
              </h2>
              <NavLink to="/timetable" className="text-[9px] sm:text-xs font-black text-slate-400 uppercase tracking-widest hover:text-indigo-600 flex items-center gap-1 transition-colors">
                View Schedule <ArrowRight size={12} className="sm:w-3.5 sm:h-3.5" />
