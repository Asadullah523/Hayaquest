...
export const TopicItem = React.memo<TopicItemProps>(({ topic }) => {
  ...
  return (
    <div 
      ...
      className={clsx(
        "group relative flex items-center justify-between p-2.5 lg:p-2.5 2xl:p-4 rounded-xl border transition-all cursor-pointer",
      ...
      <div className="flex items-center gap-3 md:gap-5 overflow-hidden relative z-0">
        {!topic.isRecurring ? (
          ...
          >
            {topic.status === 'completed' ? <CheckCircle2 className="w-5 h-5 lg:w-[18px] lg:h-[18px] 2xl:w-[22px] 2xl:h-[22px]" strokeWidth={2.5} /> : <Circle className="w-5 h-5 lg:w-[18px] lg:h-[18px] 2xl:w-[22px] 2xl:h-[22px]" strokeWidth={2} />}
          </div>
        ) : (
          <div
            className={clsx(
          ...
          >
            {isCompletedToday ? <CheckCircle2 className="w-5 h-5 lg:w-[18px] lg:h-[18px] 2xl:w-[22px] 2xl:h-[22px]" strokeWidth={2.5} /> : <Calendar className="w-5 h-5 lg:w-[18px] lg:h-[18px] 2xl:w-[22px] 2xl:h-[22px]" strokeWidth={2} />}
          </div>
        )}
        ...
        <div className="flex flex-col overflow-hidden">
            <div className="flex items-center gap-2 md:gap-3">
              <span className={clsx(
                  "text-sm lg:text-base 2xl:text-lg font-serif tracking-wide transition-all truncate",
