  ...
  persist(
    (set, get) => ({
      ...
      checkDayChange: () => {
        ...
        const { todayStats, activeSession } = get();
        
        if (todayStats.lastUpdatedDate !== today) {
          // 1. Save yesterday's accumulated stats as DB logs before wiping
          if (todayStats.totalFocusTime > 5) {
            import('./useLogStore').then(({ useLogStore }) => {
              ...
              if (Object.keys(todayStats.subjectFocusTime).length > 0) {
                Object.entries(todayStats.subjectFocusTime).forEach(([sid, seconds]) => {
                  if ((seconds as number) > 0) {
                    store.addLog({
                      date: ts, subjectId: parseInt(sid),
                      durationSeconds: seconds as number,
                      type: 'learning', timestamp: ts,
                      notes: 'Timer Session'
                    });
                  }
                });
                ...
                store.addLog({
                  date: ts, subjectId: 0,
                  durationSeconds: todayStats.totalFocusTime,
                  type: 'learning', timestamp: ts,
                  notes: 'Timer Session'
                });
              }
          ...
          // 2. IMPORTANT: If a session is active across midnight, split it!
          // We reset its counters for the new day so completeSession doesn't double-count
          // the time already saved in the "yesterday's stats" log above.
          ...
          // 3. Reset today's stats for the new day
          set({
            todayStats: {
              totalFocusTime: 0,
              totalBreakTime: 0,
      ...
      tick: () => {
        const { isActive, timerReferenceTime, timeLeftWhenStarted, mode } = get();
        ...
        // Check for day change during active timer
        get().checkDayChange();
        
        // Re-fetch state after potential day-change reset
        const stateForStats = get();
        const currentStats = stateForStats.todayStats;
        const currentSession = stateForStats.activeSession;

        if (actualTimeLeft > 0) {
          const newStats = { ...currentStats };
          
          if (mode === 'focus') {
            newStats.totalFocusTime = Math.max(0, newStats.totalFocusTime + secondsToAccountFor);
            
            // Track subject-wise time
            if (currentSession && currentSession.subjectId) {
              const sid = currentSession.subjectId;
              newStats.subjectFocusTime = { ...newStats.subjectFocusTime };
              newStats.subjectFocusTime[sid] = Math.max(0, (newStats.subjectFocusTime[sid] || 0) + secondsToAccountFor);
            }
          } else {
            newStats.totalBreakTime = Math.max(0, newStats.totalBreakTime + secondsToAccountFor);
          }
          ...
          set((s) => ({ 
            ...
            activeSession: s.activeSession ? {
                ...s.activeSession,
                elapsedSeconds: s.activeSession.elapsedSeconds + secondsToAccountFor,
                subjectDist: mode === 'focus' && s.activeSession.subjectId 
                    ? { 
                        ...s.activeSession.subjectDist, 
                        [s.activeSession.subjectId]: (s.activeSession.subjectDist[s.activeSession.subjectId] || 0) + secondsToAccountFor 
                      } 
                    : s.activeSession.subjectDist
            } : null
          }));
          ...
          if (mode === 'focus') {
            newStats.totalFocusTime = Math.max(0, newStats.totalFocusTime + finalSecondsToAccountFor);
            if (currentSession && currentSession.subjectId) {
              const sid = currentSession.subjectId;
              newStats.subjectFocusTime = { ...newStats.subjectFocusTime };
              newStats.subjectFocusTime[sid] = Math.max(0, (newStats.subjectFocusTime[sid] || 0) + finalSecondsToAccountFor);
            }
          } else {
            newStats.totalBreakTime = Math.max(0, newStats.totalBreakTime + finalSecondsToAccountFor);
          }
          ...
          set((s) => ({ 
            ...
            activeSession: s.activeSession ? {
                ...s.activeSession,
                elapsedSeconds: s.activeSession.elapsedSeconds + finalSecondsToAccountFor,
                subjectDist: mode === 'focus' && s.activeSession.subjectId 
                    ? { 
                        ...s.activeSession.subjectDist, 
                        [s.activeSession.subjectId]: (s.activeSession.subjectDist[s.activeSession.subjectId] || 0) + finalSecondsToAccountFor 
                      } 
                    : s.activeSession.subjectDist
            } : null
          }));
        ...
        if (sessionType === 'focus' && duration > 5) {
            import('./useLogStore').then(({ useLogStore }) => {
                ...
                if (Object.keys(dist).length > 0) {
                    ...
                    // No subject selected — still save so past-day analytics work
                    store.addLog({
                        date: endTime,
                        subjectId: 0,
                        durationSeconds: duration,
